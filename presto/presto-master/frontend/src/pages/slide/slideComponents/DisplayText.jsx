...
const DisplayText = ({ data, pId, sIndex, parentRef }) => {
  ...
  const [size, setSize] = useState({
    ...
    height: data?.size?.height === "auto" ? "auto" : data?.size?.height || 50 
  });
  ...
  const handleDragStart = (e) => {
    ...
    const onMouseUp = async () => {
      ...
      const { success } = await ElementPositionHandlerContext(pId, data.id, sIndex,size, position, 'text');
      if (success) {
  ...
  return (
    ...
    >
      <div
        ...
        className="border-4 min-w-[120px] overflow-hidden cursor-pointer bg-transparent rounded-md border-slate-500"
